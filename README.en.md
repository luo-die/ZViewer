...
- **Emby / Jellyfin embedded subtitle extraction (server-side demux)**: when the media server exposes no subtitle endpoint, the server demuxes the container to read embedded subtitle tracks. The first extraction of an episode takes about 1–2 minutes (then it is cached on disk and instant for every viewer); during that time the browser console prints a **real progress line every 5 seconds** (KB parsed / batch / elapsed seconds), the UI shows a one-time "extracting embedded subtitles" notice, and the server log prints `[subtitles] 提取进度 …` every 15 seconds. Partially read subtitles are delivered for playback immediately (usable within seconds) instead of waiting for the full extraction, and each request has a 30-second timeout so a dead connection errors out instead of hanging forever.
...
- **Transcode progress is synchronised**: because ffmpeg produces segments sequentially, catching up with the produced edge is no longer mistaken for a stalled picture — the host pauses and shows "server is transcoding, waiting for the next segments…", then resumes automatically once the buffer is one segment ahead (4 s by default; viewers only see the notice).
- **Session lifecycle (no more stray ffmpeg processes)**: when the movie changes, you seek, you switch back to "Auto", or a transcode source is no longer used, the client calls `POST /api/transcode/stop` so the server kills the old ffmpeg immediately. Three server-side safety nets back this up: (1) creating a session in a room stops that room's previous sessions; (2) at most 4 concurrent transcode sessions, evicting the least recently used; (3) sessions with **no playlist/segment requests for 5 minutes** are reclaimed (a playing client keeps polling the playlist, so 5 minutes of silence means nobody is watching). `GET /api/transcode/sessions` lists current sessions for diagnosis.
- **Transcode performance**:
  - **Remux instead of re-encoding whenever possible**: an `h264 + yuv420p` (8-bit) source uses `-c:v copy`, so only the container is rewritten (near-zero CPU) and only the audio is converted to AAC.
  - **Hardware encoding first**: at startup each candidate (`NVENC / QSV / AMF / VideoToolbox / VAAPI`) is validated with a real tiny test encode (encoders that are listed but unusable without a driver fail here); the first working one is used, and only otherwise does it fall back to `libx264 veryfast`. The result is cached in-process and logged as `[transcode] 检测到硬件编码器：…`. Force one with `TRANSCODE_VIDEO_ENCODER`.
  - **Hardware decoding is on by default**: when a hardware encoder is selected, decoding uses the same driver stack (`-hwaccel cuda|qsv|d3d11va|videotoolbox|vaapi`), moving decode off the CPU too; set `TRANSCODE_HWACCEL=none` to disable. If hardware decoding fails to initialise and ffmpeg exits immediately, the server **retries once without that flag** — the user just sees a few extra seconds of startup instead of a hard failure.
  - **Keyframes aligned to segments**: GOP = source fps × segment length with `-sc_threshold 0`, so every segment is cut exactly on a keyframe — segments appear sooner, playback starts sooner, seeking is more precise.
  - **No wasted work**: `-tune zerolatency` (disables encoder lookahead) and `-threads` (half the cores by default, at least 2) keep transcoding from saturating the machine; `TRANSCODE_MAX_HEIGHT=1080` downscales 4K sources to 1080p, sharply reducing decode and bandwidth cost; `TRANSCODE_SEGMENT_SECONDS` tunes the segment length (4 s by default, smaller starts faster).
  - **The source is probed once**: mode detection and frame-rate calculation share a single ffprobe result, cached by movie ID (the internal URL carries a freshly signed token every time, so the earlier URL-keyed cache never hit and the same movie was probed two or three times); replays and seeks no longer re-probe.
  - **Player-side tuning for live-style HLS**: transcode sources start at `liveSyncDurationCount=2` in hls.js (one segment less to wait for) with a 60 s forward buffer, retry late-written segments instead of failing, and the resume threshold follows the server's actual segment length so playback resumes with at least one full segment buffered.
  - Related environment variables: `FFMPEG_PATH`, `TRANSCODE_VIDEO_ENCODER`, `TRANSCODE_HWACCEL`, `TRANSCODE_VAAPI_DEVICE`, `TRANSCODE_THREADS`, `TRANSCODE_MAX_HEIGHT`, `TRANSCODE_SEGMENT_SECONDS` (see `.env.example`).
- Segments go to `config/transcode/<session id>/` and are cleaned up on shutdown; sessions are reused by "movie + start (rounded to 30 s)".
