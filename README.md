...
- 两种模式自动选择：`remux`（视频直通重封装，几乎不耗 CPU）与 `transcode`（浏览器解不了视频编码时重编码 H.264）；音轨统一转 AAC。
- 分片落在 `config/transcode/<会话 ID>/`，15 分钟无访问自动回收（进程与目录一并清理），后端退出时统一清理；会话按「影片 + 模式 + 起点(30s 取整)」复用。
