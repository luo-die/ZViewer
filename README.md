...
| 档位 | 谁可切换 | 说明 |
|---|---|---|
| **自动**（默认） | 房主/观众均可（本机偏好） | MKV / DTS 等在**浏览器内**重封装/音频转码，**零服务器 CPU 与带宽** |
| **服务端** | **仅房主**（房间级设置，全房间生效） | 服务器 ffmpeg 转 **HLS**；iPhone/iPad 走 Safari 原生 HLS，因此没有 MSE 也能看 MKV/DTS。代价是服务器 CPU 与带宽 |
| **关闭** | 房主/观众均可（本机） | 强制原生直连；MKV/DTS 等将无法播放或无声（排查用） |

- **为什么要保留「自动」**：浏览器端重封装/转码不占服务器资源，是桌面 Chrome/Edge、安卓、iPad 的默认也是最优路径（MKV、AVI/TS/WMV、DTS/AC3 音轨都靠它）。丢掉它意味着所有 MKV/DTS 片源都必须依赖服务器 ffmpeg + 全量中转带宽。
- **为什么要「服务端」**：iPhone < iOS 17.1 没有 MSE/ManagedMediaSource，浏览器端在原理上跑不起来。**当房主需要和 iOS 设备共享时**把转码方式切到「服务端」即可，全房间（含 iPhone）立刻跟着播 HLS。
- 服务端转码是**房主专属**：观众端只看到「自动 / 关闭」两档——它烧的是房主的服务器资源，不能由观众悄悄开启。观众设备播不了时，错误提示会直接引导「请房主把转码方式切到服务端」。
