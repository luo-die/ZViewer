...
- **Emby / Jellyfin 内嵌字幕提取（服务端解容器）**：媒体服务器没有字幕端点时，由服务端解容器读取内嵌字幕轨；整集首次提取约 1~2 分钟（之后落盘缓存、所有观看者秒开），期间前端**每 5 秒打印一条带真实进度的日志**（已解析多少 KB / 第几批 / 已等待多少秒），并在界面弹出一次「正在提取内嵌字幕」提示；服务端日志每 15 秒输出一条 `[subtitles] 提取进度 …`。已读到的部分字幕会先交付播放（秒级可用），不必等完整提取；单个请求带 30 秒超时，连接卡死会直接报错而不是无限等待。
...
- **播放列表一键清除**：影片列表标题栏的「清空」按钮可一次删除房间内全部影片（房主/房管可见，需二次确认，其他成员实时同步）。

### 各平台播放能力（浏览器端）

浏览器端重封装/转码依赖「把 fMP4 分片喂给 `<video>`」的能力，对应 `MediaSource`（MSE）或 iPhone 上的 `ManagedMediaSource`（MMS）：

| 平台 | 转码引擎可用性 | 说明 |
|---|---|---|
| Chrome / Edge（桌面、Android） | ✅ MSE | 完整体验，MKV / DTS 等全部可播 |
| **iPhone / iPad（iOS 17.1+）** | ✅ ManagedMediaSource | 已支持；hls.js 1.5+ 原生适配 MMS |
| iPhone（iOS < 17.1） | ❌ 无 MSE / MMS | 只能原生播放：MP4（H.264/HEVC + AAC）可播，MKV、DTS/AC3 音轨无法播放，界面会给出明确提示 |
| 桌面 Safari / Firefox | ⚠️ 部分 | MKV 原生打不开，音轨转码能力有限，建议用 Chrome / Edge |
| HLS（m3u8）源 | ✅ | Safari（含旧版 iOS）走原生 HLS，其余走 hls.js |

> iPhone 上无法在浏览器里完成转码时，播放失败提示会直接说明原因（而不是让你去拨「浏览器转码引擎」开关），并建议改用 MP4（H.264 + AAC）片源。

### 转码方式（房间级状态：房主可切，观众只读）

播放列表顶部只有一个 **「转码方式」** 控件（合并了原来的「浏览器转码引擎」与「服务端转码」）：

| 角色 | 看到什么 |
|---|---|
| **房主** | 可切换的 **自动 / 服务端** 两档（后端未装 ffmpeg 时不显示「服务端」） |
| **观众** | **只读状态**：`自动转码` 或 `服务端转码`，由房主的房间设置同步而来（进房 ack + `room-settings-updated` 广播） |

- **自动**（默认）：MKV / DTS 等在**浏览器内**重封装/音频转码，**零服务器 CPU 与带宽**。桌面 Chrome/Edge、安卓、iPad 都走这条，是成本最优路径。
- **服务端**：服务器 ffmpeg 转 **HLS**；iPhone/iPad 走 Safari 原生 HLS，因此没有 MSE 也能看 MKV/DTS。**当房主需要和 iOS 设备共享时切到这一档**，房主重新解析当前影片并把 HLS 源广播给全房间，所有成员（含 iPhone）立刻跟着播。
- 观众本机**没有**转码相关开关：服务端转码烧的是房主的服务器 CPU/带宽，不能由观众自行开启。观众设备播不了时，报错会直接提示「请房主把转码方式切到服务端」。
- **「有声音没画面」自动检测**：部分安卓设备解不了 HEVC / 10bit / AV1 视频轨，而 AAC 音轨能正常播——表现为有声音、黑屏、且不触发任何报错。现在所有引擎都带 12s 无画面看门狗，命中后会：① 向该观众明确说明是编码不受支持；② **自动上报给房主**，房主端弹出提示「某观众有声音没画面，请把转码方式切到服务端」，一键切换即可让该观众正常观看。

#### 服务端转码细节

- **模式由服务端 ffprobe 决定**（前端不再猜）：源视频是 `h264 + yuv420p`（8bit）→ `remux` 直通（零 CPU）；其余（HEVC / AV1 / 10bit / 探测失败）→ `transcode` 重编码 H.264。音轨一律转 AAC。
  > 早期版本由前端按 `movie.videoCodec` 猜模式，而挂载类片源该字段常为空 → 猜成 remux → **HEVC 原样透传**，安卓等设备切了「服务端」依旧只有声音没画面。现已在服务端用 `ffprobe` 读取源编码决定，服务器日志会打印 `[transcode] 源探测：codec=… pix_fmt=… → remux|transcode`。
- **转码进度自动同步**：ffmpeg 顺序产出分片，播放追到产出边界时不再被判成「画面卡死」重载——房主端会主动暂停并提示「服务器正在转码，等待后续片段…」，缓冲领先一个分片（默认 4 秒）后自动继续（pause/play 广播给全房间，所有人一起等、一起继续）；观众端只提示不干预。
- **转码会话生命周期（不会再留一堆后台 ffmpeg）**：切影片 / 拖进度 / 切回「自动」/ 不再使用转码源时，客户端会主动 `POST /api/transcode/stop` 让服务端立刻 kill 旧 ffmpeg；服务端另外有三层兜底——① 同房间新建会话自动回收该房间旧会话；② 同时运行的转码会话上限 4 个，超限时回收最久未访问的；③ **5 分钟无任何分片/播放列表请求**即回收（播放中的客户端会持续拉播放列表，因此 5 分钟静默 = 已没人看）。排查残留进程可访问 `GET /api/transcode/sessions`。
- **转码性能**：
  - **能直通就不转码**：源视频是 `h264 + yuv420p`（8bit）时走 `-c:v copy`（remux），只换容器、几乎零 CPU，只有音轨转 AAC。真正需要重编码的只有 HEVC / AV1 / 10bit 这类源。
  - **硬件编码优先**：启动时用一次真实的小测试编码逐项验证 `NVENC / QSV / AMF / VideoToolbox / VAAPI` 是否真的可用（能被列出但无驱动的编码器会在这里失败），可用就上 GPU，都不可用才回落 `libx264 veryfast`；结果进程内缓存，日志打印 `[transcode] 检测到硬件编码器：…`。可用 `TRANSCODE_VIDEO_ENCODER` 强制指定。
  - **硬件解码默认开启**：选定硬件编码器时用同一套驱动栈解码（`-hwaccel cuda|qsv|d3d11va|videotoolbox|vaapi`），把解码也从 CPU 挪到 GPU；`TRANSCODE_HWACCEL=none` 可关闭。若硬件解码初始化失败导致 ffmpeg 立刻退出，服务端会**自动去掉该参数重试一次**（用户侧只表现为起播慢几秒，不会直接失败）。
  - **关键帧对齐分片**：GOP = 源帧率 × 分片时长、`-sc_threshold 0`，让每个分片都在关键帧上切干净——分片产出更早、起播更快、拖动更准。
  - **不做无用功**：`-tune zerolatency`（关掉编码器 lookahead）、`-threads`（默认一半核心，至少 2）避免转码把整机 CPU 吃满；`TRANSCODE_MAX_HEIGHT=1080` 可把 4K 源降到 1080p，解码与带宽压力大幅下降；`TRANSCODE_SEGMENT_SECONDS` 可调分片时长（默认 4s，调小起播更快）。
  - **源只探测一次**：模式判定与帧率计算共用同一份 ffprobe 结果，并按「影片 ID」缓存（内部地址里带着每次现签的 token，早期实现直接拿 URL 当键等于永不命中，同一部影片会被探测两三次）；重复起播 / 拖动不再重复探测。
  - **播放端参数按直播调**：服务端转码源在 hls.js 里用 `liveSyncDurationCount=2` 起播（少等一个分片）、前向缓冲 60s 更抗抖动，分片晚写出时自动重试而不是判定播放失败；等待恢复播放的阈值也按服务端实际分片时长计算，恢复后至少还有一个完整分片可播。
  - 相关环境变量：`FFMPEG_PATH`、`TRANSCODE_VIDEO_ENCODER`、`TRANSCODE_HWACCEL`、`TRANSCODE_VAAPI_DEVICE`、`TRANSCODE_THREADS`、`TRANSCODE_MAX_HEIGHT`、`TRANSCODE_SEGMENT_SECONDS`（见 `.env.example`）。
- 分片落在 `config/transcode/<会话 ID>/`，后端退出时统一清理；会话按「影片 + 起点(30s 取整)」复用。
- **安装 ffmpeg**：让它出现在 `PATH`，或设置环境变量 `FFMPEG_PATH` 指向可执行文件。未安装时「服务端」档不会显示，接口返回 503 并给出安装提示。
- 前向 seek 受 ffmpeg 顺序产片限制——拖到尚未转出的位置需要等它追上来。
