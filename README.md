...
### 服务端转码（可选，需服务器安装 ffmpeg）

浏览器端实在解不了的源（iPhone < iOS 17.1 没有 MSE、HEVC、DTS 音轨等），可以交给服务器：

- 后端用 **ffmpeg** 把源重封装/转码为 **HLS**，前端继续用 hls.js 播放；**iOS 走 Safari 原生 HLS**，因此不依赖 MediaSource。
- 两种模式：`remux`（视频直通重封装，几乎不耗 CPU）与 `transcode`（重新编码 H.264，用于浏览器解不了视频编码时）；音轨统一转为 AAC。
- 触发方式：设备不具备浏览器端转码能力且本源必须重封装/转码时**自动兜底**；也可以在播放列表打开 **「服务端转码」** 开关强制启用（仅本机生效，切换后立即重新解析当前影片）。
- 分片落在 `config/transcode/<会话 ID>/`，15 分钟无访问自动回收（进程与目录一并清理），后端退出时统一清理。
- **安装 ffmpeg**：让它出现在 `PATH` 中，或设置环境变量 `FFMPEG_PATH` 指向可执行文件。未安装时前端不会显示该开关，接口返回 503 并给出安装提示。

> 注意：前向 seek 受 ffmpeg 顺序产片限制——拖到尚未转出的位置需要等它追上来；会话按「影片 + 模式 + 起点（30s 取整）」复用，同一房间成员共享同一份产物。服务端转码会消耗服务器 CPU 与带宽，仅在必要时开启。
